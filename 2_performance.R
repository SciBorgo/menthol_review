...
d <- read_csv("data/data_performance.csv") %>%
  clean_names() %>%
  mutate(rob = as.factor(rob))

head(d,10)

# Reverse effect time trial effect sizes
d_tt <-
  d %>%
...
# Take time to exhaustion effect sizes
d_tte <-
  d %>%
  filter(outcome != 'time_trial') %>%
  mutate(int_mu = int_x,
...
# Save forest plot
png(file = 'figures/performance.png',
    width = 10,
    height = 7,
    res = 600,
    units = "in")
forest(meta_fit, 
       sortvar = TE,
       #xlim = c(-2,2),
       xlab ="Favours Control              SMD         Favours Treatment",
...
gtsave(res_table, "results/sensitivity_perf.docx")


# Fixed effect model for limitation sections
meta_fit_fixed <-
  metacont(n.c = con_n,
                     mean.c = con_mu,
                     sd.c = con_sd,
